...
// Plus aucun code n'importe ce fichier : `lib/businessRules.ts` (seul autre
// consommateur avec lib/analytics.ts) est lui-même commenté ce jour, et
// `lib/analytics.ts` lit désormais l'instantané `nomS`/`prenomsS` porté par
// chaque participation (lib/data/rapports.ts), pas la fiche employé du jour —
// plus fidèle que `findEmployeeByMatricule` ne l'était, cf. le commentaire de
// `ParticipantRapport` dans lib/data/rapports.ts.
//
// La vraie fiche employé vit dans lib/data/employes.ts (`EmployeFiche`,
// `lireFicheEmploye`) depuis l'étape 7 (21/08/2026) — c'est elle qu'il faut
// lire, pas ce fichier.
//
// Conservé à titre d'historique (convention du projet, §0).
