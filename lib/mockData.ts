// COMMENTÉ (26/08/2026) — étape 14 du plan de migration (MODELE-DONNEES.md §13).
// Plus aucun code n'importe ce fichier : les trois rapports qui lisaient
// `mockOMs` (carte, frise, pyramide) sont passés en composants serveur et lisent
// maintenant lib/data/rapports.ts ; `lib/businessRules.ts`, qui lisait
// `getParticipationsDeEmploye`, est lui-même commenté (remplacé depuis l'étape 8
// par la détection de conflit en base, cf. lib/data/om.ts).
//
// Conservé ci-dessous à titre d'historique, comme le veut la convention du
// projet (§0) : ce fichier documentait déjà, dans ses propres commentaires, les
// défauts qu'il fallait corriger (numérotation COUNT+1, portée localStorage,
// suppression physique) — la lecture reste utile pour comprendre POURQUOI le
// modèle en base (§6-7) a pris la forme qu'il a.
//
// Commenté ligne à ligne (et non en un seul bloc /* */) : le fichier contient
// déjà des blocs /* ... */ internes (VISA, suppression) — les imbriquer aurait
// fermé le commentaire englobant au premier */ rencontré.
