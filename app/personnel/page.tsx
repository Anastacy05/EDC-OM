...
import { listerPersonnel } from "@/lib/data/employes";
 ...
 * `localStorage`. Le filtrage se fait donc en SQL, pas en JavaScript sur une
 * liste déjà transmise : avec plusieurs centaines d'employés, seule la page
...
export default async function PersonnelPage({
  ...
  searchParams: Promise<{ q?: string; statut?: string; direction?: string; inactifs?: string }>;
}) {
...
async function Tableau({
  ...
  filtres: { q?: string; statut?: string; direction?: string; inactifs?: string };
}) {
  const employes = await listerPersonnel({
    recherche: filtres.q,
    ...
    <div className={`${carteClass} overflow-x-auto p-0`}>
      ...
      <table className="w-full text-sm tabular-nums">
        <caption className="sr-only">
          Liste du personnel, {employes.length} employé{employes.length > 1 ? "s" : ""}
        </caption>
        <thead>
