...
import { missionsParAnnee, missionsParMoisDansAnnee, bornesDuMois } from "@/lib/analytics";
import { NOMS_MOIS } from "@/lib/dateUtils";
import type { MissionRapport } from "@/lib/data/rapports";
import { carteClass } from "@/lib/styles";

/**
 * Partie interactive de la page frise : mêmes principes que CarteInteractif
 * (app/rapports/carte/CarteInteractif.tsx) — données reçues en props depuis
 * le composant serveur, `useEstMonte` devenu inutile.
 */
export default function FriseInteractif({ missions }: { missions: MissionRapport[] }) {
  const router = useRouter();
  const [anneeOuverte, setAnneeOuverte] = useState<number | null>(null);

  const comptesAnnee = missionsParAnnee(missions);
  const maxAnnee = Math.max(1, ...comptesAnnee.map((c) => c.count));

  const comptesMois = anneeOuverte !== null ? missionsParMoisDansAnnee(missions, anneeOuverte) : [];
